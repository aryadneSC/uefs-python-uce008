...
def desenhar_botao_pausar_msc():
    ...
    c.screen.blit(
        txt_msc,
        (
            c.msc_botao.x + (c.msc_botao.width // 2) - (txt_msc.get_width() // 2),
            c.msc_botao.y + 35,
        ),
    )
...
def desenhar_elementos_hud(inicio_tempo, tempo_pausado_total):
    score_txt = a.assets["fonte_hud"].render(
        f"Score: {c.estado ['score']}", True, BRANCO
    )
    ...
    tempo_txt = a.assets["fonte_hud"].render(
        f"Tempo: {tempo_atual //60 :02d}:{tempo_atual %60 :02d}", True, BRANCO
    )
    ...
    c.screen.blit(
        tempo_txt, (c.WIDTH - 250, 10)
    )  # Alinhado à direita subtraindo a largura do texto
