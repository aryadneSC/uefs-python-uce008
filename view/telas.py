...
def tela_narrativa():
    while True:
        ...
        titulo_narrativa = a.assets["fonte_titulo"].render(
            "O IMPÉRIO GALÁCTICO CONTRA-ATACA", True, h.AMARELO
        )
        c.screen.blit(
            titulo_narrativa, (c.WIDTH // 2 - titulo_narrativa.get_width() // 2, 70)
        )
        ...
        for i, linha in enumerate(texto):
            ...
            c.screen.blit(
                txt, (c.WIDTH // 2 - txt.get_width() // 2, c.HEIGHT // 4 + i * 40)
            )

        instrucao = a.assets["fonte_titulo"].render(
            "Pressione ENTER para iniciar", True, h.VERDE_MUSGO
        )
        c.screen.blit(
            instrucao, (c.WIDTH // 2 - instrucao.get_width() // 2, c.HEIGHT - 70)
        )
...
def tela_menu_principal():
    ...
    dificuldade_txt = a.assets["fonte_titulo"].render(
        "[D] Dificuldade: " + c.estado["nivel_dificuldade_ativa"].capitalize(),
        True,
        h.BRANCO,
    )
    ...
    c.screen.blit(
        start_txt, (c.WIDTH // 2 - start_txt.get_width() // 2, c.HEIGHT // 2 - 40)
    )
    c.screen.blit(
        dificuldade_txt,
        (c.WIDTH // 2 - dificuldade_txt.get_width() // 2, c.HEIGHT // 2 + 10),
    )
    c.screen.blit(
        ranking_txt, (c.WIDTH // 2 - ranking_txt.get_width() // 2, c.HEIGHT // 2 + 60)
    )
    c.screen.blit(
        sair_txt, (c.WIDTH // 2 - sair_txt.get_width() // 2, c.HEIGHT // 2 + 110)
    )
...
def tela_menu_pausa():
    ...
    continuar_txt = a.assets["fonte_hud"].render(
        "Pressione P para Continuar", True, h.BRANCO
    )
    sair_txt = a.assets["fonte_hud"].render(
        "Pressione ESC para Sair", True, h.VERMELHO_PASTEL
    )

    c.screen.blit(titulo, (c.WIDTH // 2 - titulo.get_width() // 2, c.HEIGHT // 3))
    c.screen.blit(
        continuar_txt, (c.WIDTH // 2 - continuar_txt.get_width() // 2, c.HEIGHT // 2)
    )
    c.screen.blit(
        sair_txt, (c.WIDTH // 2 - sair_txt.get_width() // 2, c.HEIGHT // 2 + 50)
    )
    pygame.display.flip()
...
def tela_game_over_zona_perigo():
    ...
    detalhe = a.assets["fonte_hud"].render("Volte para trás!", True, h.BRANCO)
