...
def obter_nome_jogador ():
    ...
    while True :
        ...
        prompt = a.assets ["fonte_titulo"].render ("NOVO RECORDISTA! DIGITE SEU NOME:",True,h.AMARELO )
        txt_nome = a.assets ["fonte_titulo"].render (nome +"_",True,h.BRANCO )
        alerta = a.assets ["fonte_hud"].render ("Pressione ENTER para Salvar",True,h.VERDE_MUSGO )
...
def exibir_ranking ():
    while True :
        c.screen.fill (h.PRETO )
        titulo = a.assets ["fonte_titulo"].render ("TOP 10 - HALL DA FAMA",True,h.AMARELO )
        c.screen.blit (titulo,(WIDTH //2 -titulo.get_width ()//2,50 ))

        ranking = load_ranking ()
        if not ranking :
            vazio_txt = a.assets ["fonte_hud"].render ("NENHUM SCORE REGISTRADO AINDA.",True,h.CINZA )
        ...
        instrucao = a.assets ["fonte_titulo"].render ("PRESSIONE ENTER PARA VOLTAR",True,h.VERDE_MUSGO )
