""""
Abrange toda lógica física aplicada no jogo, como:
movimento dos stormhopers, do jogador, controle de ondas, lasers e colisões, 
...
def criar_onda(wave_idx):
    ...
    # Correção de bug: Uso do operador % para mitigar o estouro de índices que resultavam in crashes
    # Se o score do jogador for inferior a 200, as ondas se repetem ciclicamente.
    # Inspiração após discussões e orientações no fórum DevMedia.
    matriz = c.ondas[wave_idx % len(c.ondas)]
    
    # O operador varre a matriz de spawn para posicionar os Tie Fighters horizontalmente
    for i, v in enumerate(matriz):
        if v == 1:
            x = 100 + i * 80
            # Refinamento da gameplay: Escalonamento vertical aleatório para as naves não nascerem coladas
...
def gerenciar_pausa():
    ...
    while c.estado["pause"]:
        m.mostrar_menu_pausa()  # Desenha a interface estática do menu de pausa
...
def main_loop():
    ...
    while True:
        ...
        if not c.estado["vader_ativo"]: # Verifica colisao  direta guardião-inimigo
            impacto = c.chk_colisao_player_inimigos()
            if impacto:
                c.estado["vida"] -= 1
                if a.assets["sfx_dano"]:
                    a.assets["sfx_dano"].play()
                c.explosoes_ativas.append({
                    'x': impacto['x'], 'y': impacto['y'],
        ...
        atualizar_projeteis(c.tiros, -10, eh_laser_jogador=True)
        ...
        if c.estado["vader_ativo"] and not c.estado["vitoria_exibida"]:
            for t in c.tiros[:]:
                if t.colliderect(c.vader):
                    c.tiros.remove(t)
                    c.estado["vader_acertos"] += 1  # Incrementa o medidor de impactos no Boss
                    
                    # Melhoria: Gera faíscas e pequenas explosões posicionais na carcaça do Vader
                    c.explosoes_ativas.append({
                    ...
                    # Condição de vitória - 5 acertos derrotam o Darth Vader
        ...
        # Checagem avaliativo de energia do jogador (Game Over)
        if c.estado["vida"] <= 0:
            game_over()
            return
