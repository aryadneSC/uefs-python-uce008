...
def menu_principal():
    ...
    while True:
        ...
        titulo = a.fonte_titulo.render("O DESPERTAR DO GUARDIÃO", True, m.AMARELO)
        start_txt = a.fonte_titulo.render("[ENTER] Iniciar", True, m.BRANCO)
        dificuldade_txt = a.fonte_titulo.render("[D] Dificuldade: " + c.estado["nivel_dificuldade_ativa"].capitalize(), True, m.BRANCO)
        ranking_txt = a.fonte_titulo.render("[H] Hall da Fama", True, m.BRANCO)
        sair_txt = a.fonte_titulo.render("[ESC] Sair", True, m.BRANCO)
        ...
        c.screen.blit(sair_txt, (c.WIDTH // 2 - sair_txt.get_width() // 2, c.HEIGHT // 2 + 110))           # Salto de 50px
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN: # Avalia se o usuário pressionou qualquer tecla
                ...
                # Explicação: Em vez de recriar as naves e o cenário do zero a cada troca, alteramos apenas 
                # a "blueprint" (a string no dicionário de estado). A logica.py lê essa planta no momento da partida
                # e ajusta a velocidade e o multiplicador de vida dos próximos atores. 
...
if __name__ == "__main__":
    while True:
        ...
        # 1. Abre a tela estática da introdução de texto contextualizando o Guardião Rebelde.
        #    O loop do main fica congelado aqui até o jogador pressionar ENTER.
        m.tela_narrativa()    
        
        # 2. Após desempilhar a narrativa com o ENTER, abre o menu interativo.
        # Bug corrigido: Memory leaks caso o jogador apertar ESC na gameplay. Foi aplicado 'return' para limpar a memória, 
        # devolver o fluxo de forma limpa e reiniciar o ciclo de telas.
        menu_principal()